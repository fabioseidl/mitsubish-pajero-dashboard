...
static void emulator_task(void* /*param*/) {
    ...
    while (true) {
        ...
        ESP_LOGI(TAG,
                 ...
                 "fuel_rail=%.1f kPa egr_cmd=%.1f%% egr_err=%.1f%% baro=%u kPa alt=%.0fm cat=%.1fc vbat=%.2f V | "
                 "rel_tps=%.1f%% accel_d=%.1f%% accel_e=%.1f%% tps_act=%.1f%% | "
                 "stft=%.2f%% ltft=%.2f%% fp=%.1f kPa o2=%.3f abs_load=%.1f%% afr=%.3f amb=%.1fc tps_b=%.1f%% oil=%.1fc obd=%u | "
                 "gear=%.0f tgt_gear=%.0f boost=%.2f bar | flags=0x%02X",
                 ...
                 (unsigned)payload.baro_pressure_kpa, payload.altitude_m,
                 payload.catalyst_temp_c, payload.module_voltage_v,
                 ...
                 payload.throttle_b_pct, payload.oil_temp_c,
